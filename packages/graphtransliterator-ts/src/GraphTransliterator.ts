...
// =============================================================================
// SANITIZED INTERNAL TYPING SCHEMAS
// =============================================================================
...
export type GTGraph = DirectedGraph<GTNodeType, GTNodeDataType, GTEdgeDataType>;
...
function getClasses(z: string): string[] {
	...
	const regex = new RegExp(classesRegex, "g"); // Ensure your regex pattern handles global matching
...
function addRuleToken(
	...
	if (tokenNodeId !== undefined) {
		...
		let newNode: GTNode;
		[currentGraph, newNode] = addNode(currentGraph, "TokenNode", token, {
			type: "TokenData",
			token,
		});
		let newEdge: GTEdge;
	...
	if (edge) {
		...
		currentGraph = updateEdgeData(currentGraph, parentId, edge.tail, {
			cost: newCost,
		});
		return { graph: currentGraph, nodeId: edge.tail, rule };
	}
...
function orderedOutgoing(
	...
	childrenEdges.sort((a, b) => {
		...
		const nodeA = getNode(graph, a.tail);
		const nodeB = getNode(graph, b.tail);
		if (nodeA?.type_ === "RuleNode" && nodeB?.type_ === "RuleNode") {
			const ruleA =
	...
	for (const e of childrenEdges) {
		const tailNode = getNode(graph, e.tail);
		if (
			tailNode?.type_ === "TokenNode" &&
			tailNode.data.type === "TokenData" &&
			tailNode.data.token === currToken
		) {
			tokenEdges.push(e);
		} else if (tailNode?.type_ === "RuleNode") {
			ruleEdges.push(e);
		}
	}
...
/*******************************************************************************
 * Fix: Lookahead indices matched to mirror Python core pointer metrics[cite: 7]
 * Because the graph lookup pointer `tIdx` increments as graph tokens are validated,
 * lookahead matching must happen *exactly* from the current `tIdx` value forward[cite: 7].
 ******************************************************************************/
function matchConstraints(
	...
	// 4. Next Classes Lookahead (FIXED OFFSET CALCULATION)
	if (rule.nextClasses) {
		// In Python, next_classes checks immediately following next_tokens.
		// If next_tokens exists, offset by its length; otherwise use current tIdx.
...
export function matchRulesAt(
	...
	const addOutgoingToStack = (tIdx: number, nId: NodeId) => {
		...
		for (let i = ruleEdges.length - 1; i >= 0; i--) {
			stack.push({ tIdx, nodeId: ruleEdges[i].tail });
		}
		for (let i = tokenEdges.length - 1; i >= 0; i--) {
			stack.push({ tIdx: tIdx + 1, nodeId: tokenEdges[i].tail });
		}
	};
