...
// Regex matching rule syntax key string (Aligned 1:1 with Python RULE_RE)
const RULE_RE =
	/^(?:\(((?:\s?<.+?>\s+)+)?(.+?)\s?\) |((?:\s?<.+?>\s+)+)?)?((?:\n|\r|.)+?)(?:\s+\((.+?)((?:\s+<.+?>?)?)\)$|((?:\s+<.+?>)+)|$)/;
...
export function processRule(key: string, value: string): RawRuleDict {
	...
	// Groups (matches Python re.match indexing):
...
export function processOnMatchRules(
	...
	for (const item of easyreadingOnmatchRules) {
		...
		for (const prevMatch of prevStr.matchAll(ONMATCH_CLASS_RE)) {
			prevClasses.push(prevMatch[1]);
		}

		const nextClasses: string[] = [];
		for (const nextMatch of nextStr.matchAll(ONMATCH_CLASS_RE)) {
...
export function processEasyReadingSettings(
	...
	const metadataBlock =
		(settings.metadata as Record<string, string | number | boolean>) || {};
